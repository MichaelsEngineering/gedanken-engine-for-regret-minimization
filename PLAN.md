...
- Integrator: only agent allowed to merge to `main`. Owns shared/locked files and integration steps. Resolves conflicts, runs full checks, updates goldens, records decisions.
...
### Non-negotiable Rule
Workers never directly edit shared/locked files. If a shared file change is required, the worker must request the Integrator to perform it.
...
- Validation: `rg --files` shows new paths; no changes under Integrator-only files
...
### Integration Rules
- Integrator merges lanes strictly in the sequence above.
- No lane merges to `main` directly; Integrator only.
- Each lane must pass its local verification before handing off.
- Integrator runs full checks after each merge stage.
- Golden artifacts may only be updated by Integrator with explicit justification in the Decision Log and commit message.
...
- Parallel lanes: Integrator (only)
- Required inputs: all prior lane outputs
- Outputs:
  - Integrator: merged branch to `main`, updated goldens if needed, Decision Log updated
- DoD:
  - Integrator: all checks green, determinism tests passing, reproduction script verified
...
## Progress Log
Progress Log entries are Integrator-only; lanes must request the Integrator to append updates.
